...
void	ft_exit_one(t_env **env, t_blocks **blocks, int *g_exit_status)
{
...
void	ft_exit_two(t_env **env, t_blocks **blocks, int *g_exit_status)
{
	t_blocks	*tmp;
	int	cmd;
...
void	ft_check_exit(char **input, t_env **env, t_blocks **blocks, int *g_exit_status)
{
	...
	if (ft_strlen(input[0]) == 4)
	{
		...
		{
			if (len == 1)
				ft_exit_one(env, blocks, g_exit_status);
			else if (len == 2)
				ft_exit_two(env, blocks, g_exit_status);
			else if (len > 2)
				ft_exit_tree(env, blocks, g_exit_status);
		}
	}
}
